...
def task_1(compute=True, plot=False):
    ...
    if plot:
        ...
        plt.plot(l_fit, p_phase(l_fit), label=r'$fit: $' + r'$gradient=$' + r'${}\pm{}$'.format(str(round(fit_phase[0], 2)), str(round(np.sqrt(cov_phase[0][0]), 2))))
...
def task_2_b(compute=True, plot=False):
    ...
    if plot:
        ...
        plt.plot(l_fit, p_phase(l_fit), label=r'$fit: $' + r'$gradient=$' + ' ' + r'${}\pm{}$'.format(str(round(fit_phase[0], 3)), str(round(np.sqrt(cov_phase[0][0]), 3))))
...
def task_2_e():
    ...
    plt.plot(l_fit, p_phase(l_fit), label=r'$fit: $' + r'$\omega_1=$' + ' ' + r'${}\pm{}$'.format(str(round(1 - fit_phase[0], 2)), str(round(np.sqrt(cov_phase[0][0]), 2))))
    plt.xlabel(r'$\it{log({L})}$', fontname='Times New Roman', fontsize=17)
...
def task_2_f():
    ...
    plt.plot(l_fit, p_phase(l_fit), label=r'$fit: $' + r'$gradient=$' + ' ' + r'${}0\pm{}$'.format(str(round(fit_phase[0], 3)), str(round(np.sqrt(cov_phase[0][0]), 3))))
    plt.xlabel(r'$\it{log({L})}$', fontname='Times New Roman', fontsize=17)
...
def task_3_a(compute=True, plot=False):
    ...
    if plot:
        ...
        plt.plot(l_space_log, p_phase(l_space_log), label=r'$fit: \: gradient={}\pm{}$'.format(str(round(fit_phase_1[0], 2)), str(round(np.sqrt(cov_phase[0][0]), 2))))
        plt.legend()
        plt.xlabel(r'$\it{log({L})}$', fontname='Times New Roman', fontsize=17)
        ...
        plt.plot(s_space_log, p_phase(s_space_log), label=r'$fit: \: gradient\equiv-\tau={}\pm{}$'.format(str(round(fit_phase_2[0], 2)), str(round(np.sqrt(cov_phase[0][0]), 2))))
        plt.legend()
...
def task_3_b():
    ...
    plt.plot(l_log_space, p_phase_1(l_log_space), label=r'$fit: k=1,$' + ' ' + r'$gradient=$' + r'${}000\pm{}$'.format(round(gradient_1, 2), round(np.sqrt(cov_phase_1[0][0]), 4)))
    plt.plot(l_log_space, p_phase_2(l_log_space), label=r'$fit: k=2,$' + ' ' + r'$gradient=$' + r'${}\pm{}$'.format(round(gradient_2, 2), round(np.sqrt(cov_phase_2[0][0]), 2)))
    plt.plot(l_log_space, p_phase_3(l_log_space), label=r'$fit: k=3,$' + ' ' + r'$gradient=$' + r'${}0\pm{}$'.format(round(gradient_3, 2), round(np.sqrt(cov_phase_3[0][0]), 2)))
    plt.plot(l_log_space[:-80], p_phase_4(l_log_space[:-80]), label=r'$fit: k=4,$' + ' ' + r'$gradient=$' + r'${}\pm{}$'.format(round(gradient_4, 2), round(np.sqrt(cov_phase_4[0][0]), 2)))
    plt.legend()
    plt.xlabel(r'$\it{log({L})}$', fontname='Times New Roman', fontsize=17)
    ...
    err = np.array([np.sqrt(cov_phase_1[0][0]), np.sqrt(cov_phase_2[0][0]), np.sqrt(cov_phase_3[0][0]), np.sqrt(cov_phase_4[0][0])])
    gradients = [gradient_1, gradient_2, gradient_3, gradient_4]
    k_s = [1, 2, 3, 4]
    fit_phase_k, cov_phase_k = np.polyfit(k_s, gradients, 1, w=1/err, cov=True)
    ...
    plt.plot(k_space, p_phase_k(k_space), label=r'$fit:$' + r'$gradient\equiv D = $' + r'${}\pm{}$'.format(round(fit_phase_k[0], 3), round(np.sqrt(cov_phase_k[0][0]), 3)) \
                                                + '\n' + r'$fit: \tau_s = $' + r'${}\pm{}$'.format(round(1 - fit_phase_k[1]/fit_phase_k[0], 2), round(np.sqrt(cov_phase_k[1][1]+cov_phase_k[0][0]), 2)))
    plt.errorbar(k_s, gradients, yerr=err, linestyle='')
    plt.legend()
